...
class VideoPopupService:
    ...
    MAX_ATTEMPTS = (
        5  # Add max attempts definition if used elsewhere, or just rely on list
    )
    ...
    def _detect_browser(self) -> None:
        ...
        if os.name == "nt":
            ...
            for browser in self.LINUX_BROWSERS:
                path = shutil.which(browser)
                if path:
                    self._browser_path = path
                    print(f"[VideoPopup] Found Browser: {path}")
                    return

        print("[VideoPopup] No App Mode Browser found, will use default")
