...
class SkillLoader:
    ...
    def load_skills(self, force: bool = False) -> List[Dict]:
        ...
        if (
            not force
            and current_hash
            and current_hash == SkillLoader._skills_hash_cache
        ):
            ...
            try:
                ...
                if is_server_offline():
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        print(
                            f"Loaded {len(data.get('skills', []))} skills from server"
                        )
            except Exception as e:
        ...
        if data:
            if isinstance(data, dict):
                ...
                self.skills = data
                self.weapons = [
                    {"id": "common", "name": "Chung", "icon": "⭐", "color": "#95a5a6"}
                ]
            return self.skills
    ...
    def load_images(
        self, size: tuple = (40, 40), on_complete: Callable = None
    ) -> Dict[str, Any]:
        ...
        thread = threading.Thread(
            target=self._load_images_async, args=(skills_to_load, size), daemon=True
        )
        thread.start()
    ...
    def _load_images_async(self, skills: List[Dict], size: tuple):
        ...
        def load_single_image(skill: Dict) -> tuple:
            ...
            # Load from source
            try:
                if img_source.startswith("http://") or img_source.startswith(
                    "https://"
                ):
    ...
    def _load_from_file(self, filename: str) -> Optional[Image.Image]:
        ...
        paths_to_try = [
            ...
            os.path.join(
                self.resources_dir, os.path.basename(filename)
            ),  # Just filename in resources
        ]
...
class ComboPlayer:
    ...
    def play(
        self, combo_items: List[Dict], on_finish: Callable = None, loop_count: int = 1
    ):
    ...
    def _play_thread(
        self, combo_items: List[Dict], on_finish: Callable, loop_count: int
    ):
        import time
            ...
            for item in combo_items:
                ...
                if item["type"] == "delay":
                    ...
                    hold_duration = item.get(
                        "hold", 0.05
                    )  # Default 50ms if not specified
                    click_count = item.get("click_count", 1)

                    # [Anti-Cheat] Add human gap before skill if not following a delay
                    time.sleep(humanizer.get_action_delay(target_ms=35, variance_ms=10))
                ...
                elif item["type"] in ("sequence_key", "macro_key"):
                    # [Anti-Cheat] Add human gap before key press
                    time.sleep(humanizer.get_action_delay(target_ms=30, variance_ms=8))
                ...
                elif item["type"] in ("sequence_mouse", "macro_mouse"):
                    # [Anti-Cheat] Add human gap before mouse action
                    time.sleep(humanizer.get_action_delay(target_ms=30, variance_ms=8))
    ...
    def _execute_skill(
        self, key_str: str, hold_duration: float = 0.05, modifiers: list = None
    ):
...
class ComboManager:
    ...
    def _normalize_trigger(key_or_button: Any) -> str:
        ...
        Also handles combo triggers (tuple of modifiers + main key).
        """
        # Handle combo trigger tuple: (frozenset of modifiers, main key)
        if isinstance(key_or_button, tuple) and len(key_or_button) == 2:
            modifiers, main_key = key_or_button
            if isinstance(modifiers, frozenset):
                mod_str = "+".join(sorted(modifiers))
                key_str = ComboManager._normalize_single_key(main_key)
                return f"{mod_str}+{key_str}" if mod_str else key_str

        return ComboManager._normalize_single_key(key_or_button)

    @staticmethod
    def _normalize_single_key(key_or_button: Any) -> str:
        """Normalize a single key/button to string"""
...
class TriggerManager:
    """Manages global keyboard/mouse listeners for triggers with combo key support"""
    ...
    def _set_trigger(self, key_or_button):
        """Set the trigger key/button with optional modifiers"""
        ...
        if self.on_trigger_set:
            self.on_trigger_set(self.trigger_key_code)

    def matches_trigger(self, key_or_button) -> bool:
        """Check if key/button matches the current trigger (with modifiers)"""
        if isinstance(self.trigger_key_code, tuple):
            required_mods, main_key = self.trigger_key_code
        ...
        return self._keys_equal(key_or_button, self.trigger_key_code)

    def _keys_equal(self, key1, key2) -> bool:
        """Compare two keys for equality (handles different key object types)"""
        # Direct comparison
        if key1 == key2:
            return True
        # Compare by string representation for KeyCode objects
        return str(key1) == str(key2)

    @staticmethod
    def get_key_name(trigger) -> str:
        ...
        if isinstance(trigger, tuple):
            ...
            key_name = TriggerManager._get_single_key_name(main_key)
            return "+".join(mod_names + [key_name])
        return TriggerManager._get_single_key_name(trigger)

    @staticmethod
    def _get_single_key_name(key) -> str:
    ...
    def parse_trigger_string(trigger_str: str):
        ...
        try:
            ...
            if "+" in trigger_str:
                ...
                main_key = TriggerManager._parse_single_key(parts[-1])
                return (modifiers, main_key)

            return TriggerManager._parse_single_key(trigger_str)
        except Exception:
            return mouse.Button.x1

    @staticmethod
    def _parse_single_key(key_str: str):
...
class TemplateManager:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if synced_count > 0:
                    self.save_templates()
                    print(
                        f"[TemplateManager] Synced {synced_count} new templates from server"
                    )

                self.server_templates = new_server_templates
                print(
                    f"[TemplateManager] Fetched {len(new_server_templates)} templates from server"
                )
            else:
    ...
    def save_templates(self) -> bool:
        ...
        try:
            with open(self.templates_file, "w", encoding="utf-8") as f:
                json.dump(
                    {"templates": self.templates}, f, indent=2, ensure_ascii=False
                )
            return True
        except Exception as e:
    ...
    def add_template(self, name: str, items: List[Dict]) -> bool:
        ...
        self.templates.append({"name": name, "items": items})
