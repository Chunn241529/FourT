...
from core.config import VERSION_FILE, get_license_server_url
from utils import get_current_version, compare_versions, is_frozen, get_app_directory


def get_update_server_url() -> str:
    """Get dynamic update server URL (always up-to-date)"""
    return f"{get_license_server_url()}/update/info"
...
class UpdateService:
    ...
    def check_for_updates_sync(self) -> bool:
        ...
        try:
            ...
            # Fetch update info from server (use dynamic URL)
            update_url = get_update_server_url()
            data = self._fetch_json(update_url, timeout=10)
            if not data:
            ...
            if self.installer_url and not self.installer_url.startswith("http"):
                ...
                self.installer_url = urljoin(update_url, self.installer_url)
    ...
    def download_update_sync(self) -> bool:
        ...
        try:
            ...
            self.installer_path = os.path.join(
                self.download_dir, f"FourT_Setup_{self.new_version}.exe"
            )
    ...
    def _fetch_json(self, url: str, timeout: int = 10) -> Optional[dict]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                print(
                    f"[Update] Fetch attempt {attempt + 1}/{self.max_retries} failed: {e}"
                )
    ...
    def _download_file(self, url: str, destination: str) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Get file size
                req = urllib.request.Request(
                    url, method="HEAD", headers={"User-Agent": "FourT-Helper/1.0"}
                )
