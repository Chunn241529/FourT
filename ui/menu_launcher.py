...
class MenuLauncher:
    ...
    def show_menu(self, event):
        ...
        has_sequence = (
            self.feature_manager.has_feature(Features.SEQUENCE)
        ...
        def get_item_state(feature_id: str) -> dict:
            """Get state dict: {visible, command, suffix}"""
            config = menu_items_config.get(feature_id, {})
            visible = config.get("visible", True)

            state = {"visible": visible, "command": None, "suffix": ""}

            if config.get("maintenance", False):
                state["command"] = lambda: self.show_maintenance_dialog(
                    config.get("label", feature_id)
                )
                state["suffix"] = " ⚠️"
            elif config.get("in_develop", False):
                state["command"] = lambda: self.show_development_dialog(
                    config.get("label", feature_id)
                )
                state["suffix"] = " 🚧"
        ...
        # MIDI Auto Player
        midi_state = get_item_state("midi_playback")
        if midi_state["visible"]:
            if midi_state["command"]:  # Maintenance/Dev
                cmd = midi_state["command"]
                icon = "🎹"
                fg = "#f38ba8" if "⚠️" in midi_state["suffix"] else "#89b4fa"
            elif self.feature_manager.has_feature(Features.MIDI_PLAYBACK):
                cmd = self.open_midi_player
                icon = "🎹"
                fg = "#a6e3a1"
            else:
                cmd = self.show_midi_restriction
                icon = "🔒"
                fg = "#6c7086"

            menu_items.append(
                {
                    "label": t("auto_play_midi") + midi_state["suffix"],
                    "command": cmd,
                    "icon": icon,
                    "fg": fg,
                }
        ...
        quest_state = get_item_state("quest_video_helper")
        if quest_state["visible"]:
            if quest_state["command"]:
                cmd = quest_state["command"]
                icon = "🎯"
                fg = "#f38ba8" if "⚠️" in quest_state["suffix"] else "#89b4fa"
            elif self.feature_manager.has_feature(Features.QUEST_VIDEO_HELPER):
                cmd = self.open_quest_video_helper
                icon = "🎯"
                fg = "#89dceb"
            else:
                cmd = self.show_quest_helper_restriction
                icon = "🔒"
                fg = "#6c7086"

            menu_items.append(
                {
                    "label": t("quest_video_helper") + quest_state["suffix"],
                    "command": cmd,
                    "icon": icon,
                    "fg": fg,
                }
        ...
        ping_state = get_item_state("ping_optimizer")
        if ping_state["visible"]:
            if ping_state["command"]:
                cmd = ping_state["command"]
                icon = "⚡"
                fg = "#f38ba8" if "⚠️" in ping_state["suffix"] else "#89b4fa"
            elif self.feature_manager.has_feature(Features.PING_OPTIMIZER):
                cmd = self.open_ping_optimizer
                icon = "⚡"
                fg = "#cba6f7"
            else:
                cmd = self.show_ping_optimizer_restriction
                icon = "🔒"
                fg = "#6c7086"

            menu_items.append(
                {
                    "label": t("ping_optimizer") + ping_state["suffix"],
                    "command": cmd,
                    "icon": icon,
                    "fg": fg,
                }
        ...
        trans_state = get_item_state("screen_translator")
        if trans_state["visible"]:
            if trans_state["command"]:
                cmd = trans_state["command"]
                icon = "🌐"
                fg = "#f38ba8" if "⚠️" in trans_state["suffix"] else "#89b4fa"
            elif self.feature_manager.has_feature(Features.SCREEN_TRANSLATOR):
                cmd = self.open_screen_translator
                icon = "🌐"
                fg = "#94e2d5"
            else:
                cmd = self.show_screen_translator_restriction
                icon = "🔒"
                fg = "#6c7086"

            menu_items.append(
                {
                    "label": t("screen_translator") + trans_state["suffix"],
                    "command": cmd,
                    "icon": icon,
                    "fg": fg,
                }
        ...
        # WWM Combo (PRO feature)
        wwm_state = get_item_state("wwm_combo")
        if wwm_state["visible"]:
            if wwm_state["command"]:
                cmd = wwm_state["command"]
                icon = "⚔"
                fg = "#f38ba8" if "⚠️" in wwm_state["suffix"] else "#89b4fa"
            elif self.feature_manager.has_feature(Features.WWM_COMBO):
                cmd = self.open_wwm_combo
                icon = "⚔"
                fg = "#c6a0f6"
            else:
                cmd = self.show_wwm_restriction
                icon = "🔒"
                fg = "#6c7086"

            menu_items.append(
                {
                    "label": t("sequence_combo") + wwm_state["suffix"],
                    "command": cmd,
                    "icon": icon,
                    "fg": fg,
                }
    ...
    def open_midi_player(self):
        ...
        if self.midi_window and tk.Toplevel.winfo_exists(self.midi_window):
            ...
            self.midi_window.geometry("505x480")
    ...
    def show_sequence_restriction(self):
        """Show message when free user tries to access sequence"""
    ...
    def show_maintenance_dialog(self, feature_name):
        """Show modern maintenance dialog"""
        from .components import FramelessWindow

        # Use t() for translations
        title_text = feature_name if " " in feature_name else t(feature_name)
        if title_text == feature_name:  # If translation missing
            if feature_name == "auto_play_midi":
                title_text = "Auto Music"
            elif feature_name == "quest_video_helper":
                title_text = "Quest Helper"

        # Note: A full custom dialog would be better but requires more code.
        # Given "modern dialog" request, let's stick to standard first to ensure logic works,
        # or implement a simple custom one if user insists.
        # User said "dialog UI hien dai" -> MUST use custom UI.
        self._show_custom_notice(
            t("maintenance_title"),
            t("maintenance_msg", feature=title_text),
            "#e67e22",
        )

    def show_development_dialog(self, feature_name):
        """Show modern development dialog"""
        title_text = feature_name if " " in feature_name else t(feature_name)
        self._show_custom_notice(
            t("develop_title"),
            t("develop_msg", feature=title_text),
            "#9b59b6",
        )

    def _show_custom_notice(self, title, message, color):
        """Show a modern custom notice dialog"""
        if self.notice_dialog and tk.Toplevel.winfo_exists(self.notice_dialog):
            self.notice_dialog.lift()
            self.notice_dialog.focus_force()
            return

        from .components import FramelessWindow

        dlg = FramelessWindow(self.root, title=title, width=380, height=200)
        self.notice_dialog = dlg
        dlg.center_relative(self.root)

        # Content
        frame = dlg.content_frame

        # Title
        tk.Label(
            frame, text=title, font=FONTS["h2"], bg=colors["bg"], fg=colors["fg"]
        ).pack()
        ...
        ModernButton(
            frame, text=t("understood"), command=dlg.destroy, kind="primary", width=12
        ).pack(pady=10)

        # Fade in
        from .animations import FadeEffect

        FadeEffect.fade_in(dlg)
