...
class MenuLauncher:
    ...
    def draw_button(self, hover=False, scale=1.0):
        ...
        self.canvas.create_arc(
            ...
            center + highlight_radius - highlight_offset,
            start=45, extent=90,
            fill='', outline='#ffffff' if hover else '#404060',
            width=1, style='arc'
        )
    ...
    def on_drag(self, event):
        ...
        distance = math.sqrt((event.x - self.drag_x)**2 + (event.y - self.drag_y)**2)
    ...
    def show_menu(self, event):
        ...
        has_macro = self.feature_manager.has_feature(Features.MACRO) if self.feature_manager else False
        
        # Prepare menu items
        menu_items = []
        
        # Trial Status (use cached values - already loaded at splash screen)
        if self.feature_manager.current_package == 'free' and self.feature_manager.trial_active:
            remaining = self.feature_manager.trial_remaining_seconds
            minutes = remaining // 60
            menu_items.append({
                'label': f"TRIAL: {minutes} phút còn lại",
                'command': None,
                'icon': "⏱",
                'fg': '#fab387'
            })
        ...
        if x < 10: x = 10
        if x + menu_width > screen_width - 10: x = screen_width - menu_width - 10
    ...
    def open_midi_player(self):
        ...
        if self.midi_window and tk.Toplevel.winfo_exists(self.midi_window):
            ...
            self.midi_window = FramelessWindow(self.root, title="MIDI Auto Player", icon_path=icon_path)
    ...
    def _get_server_packages(self):
        """Get packages from server (with caching)"""
    ...
    def show_feature_restriction(self, feature: str, feature_name: str, description: str = ""):
        """
        ...
        msg = f"{feature_name} là tính năng của gói {min_package} trở lên."
        if description:
            msg += f"\n\n{description}"
        msg += "\n\nBạn có muốn nâng cấp ngay không?"
        
        response = messagebox.askyesno(
            f"Tính năng {feature_name}",
            msg,
            icon='info'
        )
        if response:
    ...
    def open_macro_recorder(self):
        ...
        if self.macro_window and tk.Toplevel.winfo_exists(self.macro_window):
            ...
            from .components import FramelessWindow
            icon_path = _get_icon_path()
            self.macro_window = FramelessWindow(self.root, title="Macro Recorder", icon_path=icon_path)
    ...
    def open_quest_video_helper(self):
        ...
        if self.quest_helper_window:
            try:
                if self.quest_helper_window.root and tk.Toplevel.winfo_exists(self.quest_helper_window.root):
    ...
    def open_ping_optimizer(self):
        ...
        if self.ping_optimizer_window and tk.Toplevel.winfo_exists(self.ping_optimizer_window):
            self.ping_optimizer_window.lift()
            self.ping_optimizer_window.focus_force()
        else:
            from .components import FramelessWindow
            icon_path = _get_icon_path()
            self.ping_optimizer_window = FramelessWindow(self.root, title="Ping Optimizer", icon_path=icon_path)
    ...
    def _show_update_splash(self, update_info):
        ...
        try:
            ...
            def on_progress(current_mb, total_mb, percent):
                print(f"[Update] Downloading: {current_mb:.1f}MB / {total_mb:.1f}MB ({percent:.0f}%)")
    ...
    def open_upgrade(self):
        ...
        if self.upgrade_window:
            try:
                if hasattr(self.upgrade_window, 'window') and tk.Toplevel.winfo_exists(self.upgrade_window.window):
