...
class ScriptViewerWindow(FramelessWindow):
    ...
    def __init__(
        self, parent, events, midi_path, debug_info=None, on_play=None, on_save=None
    ):
    ...
    def _create_widgets(self):
        ...
        if self.debug_info:
            ...
            tk.Label(
                info_row1,
                text=self.debug_info.get("estimated_key", "N/A"),
            ...
            tk.Label(
                info_row1,
                text=f"{self.debug_info.get('transpose', 0):+d} semitones",
                font=("Segoe UI", 10, "bold"),
                bg=colors["card"],
                fg=colors["fg"],
            ...
            out_color = (
                colors["success"]
                if out_pct < 5
                else (colors["warning"] if out_pct < 20 else colors["error"])
            )
            tk.Label(
                info_row2,
                text="📊 Out of range:",
            ...
            tk.Label(
                info_row2,
                text=f"{out_range}/{total} ({out_pct:.1f}%)",
                font=("Segoe UI", 10, "bold"),
                bg=colors["card"],
                fg=out_color,
            ...
            self.transpose_var = tk.StringVar(
                value=str(self.debug_info.get("transpose", 0))
            )
            self.transpose_entry = tk.Entry(
                info_row3,
            ...
            ModernButton(
                info_row3,
                text="Reprocess",
                command=self._reprocess_with_transpose,
                kind="accent",
                width=10,
        ...
        columns = ("idx", "time", "action", "key", "modifier", "hand")
        ...
        self.tree.column("modifier", width=80, anchor="center")
        self.tree.column("hand", width=60, anchor="center")
    ...
    def _populate_script(self):
        ...
        for idx, event in enumerate(self.events):
            ...
            if len(event) == 5:
                time, action, key, modifier, hand = event
            else:
            ...
            hand_str = "High (R)" if hand == 1 else "Low (L)"
    ...
    def _on_double_click(self, event):
        ...
        if len(current_event) == 5:
            time, action, key, modifier, hand = current_event
        else:
        ...
        if col == "#2":  # Time column
            ...
            if new_val is not None:
                self.events[row_idx] = (new_val, action, key, modifier, hand)
                self._mark_modified()
            ...
            if new_val and len(new_val) == 1:
                self.events[row_idx] = (time, action, new_val.lower(), modifier, hand)
                self._mark_modified()
            ...
            def set_mod(mod):
                self.events[row_idx] = (time, action, key, mod, hand)
                self._mark_modified()
                mod_window.destroy()

            for mod_text, mod_val in [
                ("Không có", None),
                ("Shift (♯)", "shift"),
                ("Ctrl (♭)", "ctrl"),
            ]:
                btn = ModernButton(
                    mod_window,
                    text=mod_text,
                    command=lambda m=mod_val: set_mod(m),
                    kind="secondary" if mod_val != modifier else "accent",
                    width=15,
                )
                btn.pack(pady=3)
        elif col == "#6":  # Hand column
            # Toggle hand
            new_hand = 1 if hand == 0 else 0
            self.events[row_idx] = (time, action, key, modifier, new_hand)
            self._mark_modified()
            self._populate_script()
    ...
    def _on_delete(self, event):
        ...
        if len(deleted) == 5:
            time, action, key, modifier, hand = deleted
        else:
        ...
        if paired_idx is not None:
            if messagebox.askyesno(
                "Xóa Event", f"Xóa cả cặp press/release cho phím '{key}'?"
            ):
    ...
    def _reprocess_with_transpose(self):
        ...
        if use_auto:
            ...
            try:
                ...
                messagebox.showerror(
                    "Lỗi", "Transpose phải là số nguyên (VD: -2, 0, 3)"
                )
                return
        ...
        try:
            ...
            if use_auto:
                messagebox.showinfo(
                    "✓",
                    f"Đã reprocess với Auto Transpose (= {new_debug_info.get('transpose', 0):+d})",
                )
            else:
                messagebox.showinfo(
                    "✓", f"Đã reprocess với transpose = {manual_transpose:+d}"
                )
