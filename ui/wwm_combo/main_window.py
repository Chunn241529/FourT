...
from .sequence_record_dialog import SequenceRecordDialog as MacroRecordDialog
...
class WWMComboWindow(FramelessWindow):
    ...
    def _setup_editor(self, parent):
        ...
        ModernButton(
            controls, text="🗑 Clear", command=self.clear_combo, kind="secondary"
        ).pack(side="right", padx=5)
    ...
    def _setup_activity(self, parent):
        ...
        ModernButton(
            header, text="Clear", command=self.clear_active_combos, kind="secondary"
        ).pack(side="right", padx=5, pady=5)
        ...
        ModernButton(
            load_frame, text="📂 Load Combo", command=self.load_combo, kind="secondary"
        ).pack(fill="x", padx=5)
    ...
    def _play_combo(self, combo_data):
        ...
        self.player.play(
            filtered_items,
            on_finish=lambda: self.status_var.set("Ready"),
