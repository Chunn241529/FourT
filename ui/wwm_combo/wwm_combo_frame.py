...
from .sequence_record_dialog import SequenceRecordDialog
...
class WWMComboFrame(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self._recording_dialog_open = False
    ...
    def _setup_bottom_controls(self, parent):
        ...
        # Safety Warning
        warning_frame = tk.Frame(parent, bg=colors["bg"])
    ...
    def _render_macro_item(self, x, y, h, idx, item, tag):
        ...
        # Color based on type
        if item["type"] in ("macro_key", "sequence_key"):
    ...
    def _open_record_dialog(self):
        ...
        if self._recording_dialog_open:
            return

        self._recording_dialog_open = True
        SequenceRecordDialog(self, on_add_callback=self._on_record_complete)

        # Reset flag when dialog closes (approximate)
        self.after(1000, lambda: setattr(self, "_recording_dialog_open", False))
