...
class WWMComboFrame(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.trigger_key_var = tk.StringVar(value=session.get('trigger_key', "Button.x1"))
        self.playback_mode_var = tk.StringVar(value=session.get('playback_mode', "once"))
        ...
        self.trigger_manager.trigger_key_code = TriggerManager.parse_trigger_string(trigger_key)
    ...
    def _save_session_state(self):
        ...
        current_weapon = self.current_weapon_var.get() if hasattr(self, 'current_weapon_var') else ""
    ...
    def _render_palette_skill(self, skill: dict, y: int, width: int, row_height: int) -> int:
        ...
        if not skill_id_lower.startswith('mystic_skill_') and not skill_id_lower.startswith('common_'):
            # skill_1 and skill_3 (perception) both use Q slot
            if skill_id_lower.endswith('_skill_1') or skill_id_lower.endswith('_skill_3'):
                keybind_key = 'skill_1'
            # skill_2 and skill_4 (perception) both use E/` slot
            elif skill_id_lower.endswith('_skill_2') or skill_id_lower.endswith('_skill_4'):
                keybind_key = 'skill_2'
    ...
    def _play_combo(self, combo_data):
        ...
        self.player.play(items, on_finish=lambda: self.status_var.set("Ready"), loop_count=loop_count)
