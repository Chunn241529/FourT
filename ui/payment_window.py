...
class AnimatedQRFrame(Canvas):
    ...
    def _draw_frame(self):
        ...
        if self._loading:
            ...
            glow_color = interpolate_color(COLORS['bg'], COLORS['accent'], glow_intensity)
            
            # Outer glow
            draw_rounded_rect(
                self, 5, 5, self.size + 15, self.size + 15,
                radius=16,
                fill=glow_color,
                outline=''
            )
            ...
            # Inner frame
            draw_rounded_rect(
                self, 10, 10, self.size + 10, self.size + 10,
                radius=12,
                fill='white',
            ...
            draw_rounded_rect(
                self, 8, 8, self.size + 12, self.size + 12,
                radius=12,
                fill='white',
    ...
    def _start_loading_animation(self):
        ...
        self._animation_id = self.after(80, self._start_loading_animation)  # Slower animation
    ...
    def set_error(self, message="Lỗi"):
        ...
        self.create_text(cx, cy, text=f"❌ {message}", font=("Segoe UI", 12), fill=COLORS['error'])
...
class PaymentWindow:
    ...
    def __init__(self, parent, feature_manager, package_info, on_success=None):
        ...
        self.window = FramelessWindow(parent, title=f"Thanh toán - {package_info['name']}")
    ...
    def _init_payment(self):
        ...
        def task():
            ...
            self.order_data = self.feature_manager.create_payment_order(
                ...
                amount=self.package_info.get('price', 0)  # Use price from server config
            )
            
            if not self.order_data:
                self.window.after(0, lambda: messagebox.showerror("Lỗi", "Không thể tạo đơn hàng"))
            ...
            if SEPAY_ENABLED and not is_offline:
                self.window.after(0, lambda: self.auto_check_label.config(
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("payment_verified"):
                        ...
                        if license_key:
                            self.window.after(0, lambda: self._handle_payment_success(license_key))
                        return
                    else:
                        self.window.after(0, lambda: self.auto_check_label.config(
