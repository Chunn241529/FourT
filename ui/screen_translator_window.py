...
class TranslationOverlay(tk.Toplevel):
    ...
    def position_near_region(self, x: int, y: int, w: int, h: int):
        ...
        # 1. Resize logic: Match width to selection
        # Padding is ~32px (16 left + 16 right)
        wrap_width = max(50, w - 32)
        self.text_label.configure(wraplength=wrap_width)

        # Calculate required height with new wrap length
        self.update_idletasks()
        req_h = self.winfo_reqheight()

        # Height: Use at least the selection height, but expand if text is long
        final_h = max(h, req_h)
        final_w = w

        # 2. Position logic: Farther away (Increased offset)
        OFFSET = 50  # Increased from 10 to 50 as requested ("xa hơn nhiều")

        overlay_x = x
        overlay_y = y + h + OFFSET

        # Keep on screen
        screen_w = self.winfo_screenwidth()
        screen_h = self.winfo_screenheight()

        if overlay_x + final_w > screen_w:
            overlay_x = screen_w - final_w - 10

        if overlay_y + final_h > screen_h:
            # If bottom results in overflow, put it ABOVE the region
            overlay_y = y - final_h - OFFSET

        # Ensure y is not negative
        if overlay_y < 0:
            overlay_y = 10

        self.geometry(f"{final_w}x{final_h}+{overlay_x}+{overlay_y}")
...
class ScreenTranslatorWindow:
    ...
    def _save_settings(self):
        ...
        try:
            settings = {
                ...
                "ocr_engine": self.engine_map_name_to_id.get(
                    self.engine_var.get(), "windows"
                ),
    ...
    def _create_window(self):
        ...
        # Build engine list dynamically
        self.engine_ids = self.ocr_manager.get_available_engines()
        self.engine_names = [
            self.ocr_manager.get_engine_info(eid).get("name", eid)
            for eid in self.engine_ids
        ]

        # Map for display/save
        self.engine_map_name_to_id = dict(zip(self.engine_names, self.engine_ids))
        self.engine_map_id_to_name = dict(zip(self.engine_ids, self.engine_names))

        # Set initial value by name
        current_id = self._settings.get("ocr_engine", "windows")
        current_name = self.engine_map_id_to_name.get(current_id, "Windows OCR")
        self.engine_var.set(current_name)

        engine_cb = ttk.Combobox(
            ocr_row,
            textvariable=self.engine_var,
            values=self.engine_names,
            width=20,
            state="readonly",
        )
    ...
    def _on_engine_change(self, event=None):
        ...
        engine_id = self.engine_map_name_to_id.get(engine_name, "windows")
        self.ocr_manager.set_engine(engine_id)
        self._update_ocr_status()
        self._save_settings()
    ...
    def _update_ocr_status(self):
        ...
        try:
            engine_name = self.engine_var.get()
            engine_id = (
                self.engine_map_name_to_id.get(engine_name, "windows")
                if hasattr(self, "engine_map_name_to_id")
                else "windows"
            )
            ...
            if is_ready:
                ...
                engine_name = self.engine_map_id_to_name.get(engine_id, engine_id)
    ...
    def _open_ocr_setup(self):
        ...
        selected_engine_name = self.engine_var.get()
        selected_engine_id = self.engine_map_name_to_id.get(
            selected_engine_name, "windows"
        )
        OCRSetupWindow(self.root, on_ready=on_ready, selected_engine=selected_engine_id)
    ...
    def _run_realtime(self, x: int, y: int, w: int, h: int):
        """Run realtime translation loop (Async Version)"""
        ...
        def on_translation_done(req_id, result_text, detected_lang, original_text):
            """Callback when async translation finishes"""
            # Check if this request is still valid (matches current ID)
            if req_id != self._current_request_id or self._stop_realtime:
                # print(f"DEBUG: Discarding outdated result {req_id} (Current: {self._current_request_id})")
                return
            ...
            self.root.after(
                ...
                lambda: self._update_ui_with_result(
                    original_text, result_text, detected_lang
                ),
            )
        ...
        def loop():
            ...
            while not self._stop_realtime:
                ...
                try:
                    ...
                    if text and text.strip() and is_valid_text(text):
                        ...
                        if self._text_significantly_different(
                            ...
                            # 1. Update Request ID (Invalidates previous pending requests)
                            self._current_request_id += 1
                            current_req_id = self._current_request_id

                            # 2. Update Status only (Don't clear overlay with "..." anymore)
                            self.realtime_status.configure(
                                text="⏳ Đang tải bản dịch mới...", fg=colors["warning"]
                            )

                            # self.root.after(
                            #     0,
                            #     lambda t=text_clean: self.overlay.show_text(
                            #         t, "...", ""
                            #     ),
                            # )
                            ...
                            if self.smart_mode_var.get():
                                ...
                                self.translation_service.translate_smart_async(
                                    text_clean,
                                    callback=lambda res, lang: on_translation_done(
                                        current_req_id, res, lang, text_clean
                                    ),
                                    context=context,
                                    dest=self.target_lang,
                                    style="volam",
                                ...
                                # Normal translation (also async wrapper needed or just run caching logic)
                                # For consistency, we use the updated async flow for all
                ...
                # === Loop Optimization & Stealth ===
                # 1. Calculate how long processing took
                processing_ms = (time.time() - loop_start) * 1000

                # 2. Subtract processing time from interval (Drift correction)
                remaining_ms = max(0, interval_ms - processing_ms)

                # 3. Add Randomized Jitter (Stealth)
                sleep_ms = stealth_manager.get_random_delay(remaining_ms)

                # Ensure minimum sleep to be safe
                sleep_sec = max(0.05, sleep_ms / 1000.0)

                # Sleep in small chunks
                steps = int(sleep_sec / 0.1)
                ...
                # Sleep remainder
                if not self._stop_realtime:
                    remainder = sleep_sec - (steps * 0.1)
                    if remainder > 0:
    ...
    def _update_ui_with_result(self, original, translated, lang):
        """Helper to update UI from main thread"""
        if translated and len(translated.strip()) >= 1:
            # Basic fuzzy check to avoid flickering same text
            if self._text_significantly_different(translated, self._last_translated):
                self._last_translated = translated
                self.overlay.show_text(original, translated, lang)
                self._add_to_log(original, translated)
    ...
    def _text_significantly_different(self, new_text: str, old_text: str) -> bool:
        ...
        if total > 0:
            ...
            # Lower threshold to 0.8 to be less sensitive to OCR noise
        ...
        if old_words and new_words:
            ...
            if smaller_set > 0 and intersection / smaller_set >= 0.8:
                return False
