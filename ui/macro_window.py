...
class MacroLibraryWindow(tk.Toplevel):
    ...
    def setup_ui(self):
        ...
        tk.Label(header, text="Macro Library", font=FONTS['h2'], 
                bg=colors['header'], fg=colors['fg']).pack(side='left', padx=10)
        
        # List
        list_frame = tk.Frame(self, bg=colors['bg'])
        list_frame.pack(fill='both', expand=True, padx=10, pady=10)
        
        self.listbox = tk.Listbox(list_frame, bg=colors['input_bg'], fg=colors['fg'],
                                  font=FONTS['body'], selectmode='single',
        ...
        ModernButton(btn_frame, text="Load", command=self.load_selected,
                    kind='primary').pack(side='right', padx=5)
...
class MacroQuickPanel(tk.Frame):
    ...
    def __init__(self, parent, on_load_callback, on_play_callback, on_delete_callback=None, **kwargs):
        ...
        self.macros_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "macros")
    ...
    def setup_ui(self):
        ...
        tk.Label(header, text="📚 Library", font=FONTS['bold'], 
                bg=colors['header'], fg=colors['fg']).pack(side='left', padx=8, pady=5)
        
        # Refresh button
        refresh_btn = tk.Label(header, text="🔄", font=FONTS['body'], 
                              bg=colors['header'], fg=colors['fg'], cursor='hand2')
    ...
    def refresh_list(self):
        ...
        if not macro_files:
            # Empty state
            tk.Label(self.items_frame, text="Chưa có macro nào", 
    ...
    def _create_item(self, filename, filepath, trigger):
            ...
            for child in item.winfo_children():
                ...
                for subchild in child.winfo_children():
                    if hasattr(subchild, 'configure'):
                        try: subchild.configure(bg=hover_bg)
                        except: pass
        ...
        def on_leave(e):
            ...
            for child in item.winfo_children():
                ...
                for subchild in child.winfo_children():
                    if hasattr(subchild, 'configure'):
                        try: subchild.configure(bg=card_bg)
                        except: pass
        ...
        content.bind('<Double-Button-1>', lambda e, fp=filepath: self._on_double_click(fp))
        
        # Macro name (without .json)
        name = filename.replace('.json', '')
        name_label = tk.Label(content, text=name, font=FONTS['bold'], 
                             bg=card_bg, fg=colors['fg'], anchor='w')
        name_label.pack(fill='x')
        name_label.bind('<Button-1>', lambda e, fp=filepath: self._on_single_click(fp))
        name_label.bind('<Double-Button-1>', lambda e, fp=filepath: self._on_double_click(fp))
        
        # Trigger info (if set)
        if trigger:
            trigger_label = tk.Label(content, text=f"⌨ {trigger}", font=FONTS['small'],
                                    bg=card_bg, fg=colors['accent'])
            trigger_label.pack(anchor='w', pady=(2, 0))
            trigger_label.bind('<Button-1>', lambda e, fp=filepath: self._on_single_click(fp))
            trigger_label.bind('<Double-Button-1>', lambda e, fp=filepath: self._on_double_click(fp))
        ...
        delete_btn = tk.Label(btn_frame, text="✕", font=FONTS['small'], 
                             bg=card_bg, fg=colors['fg_dim'], cursor='hand2')
        ...
        delete_btn.bind('<Button-1>', lambda e, fp=filepath, fn=filename: self._on_delete(fp, fn))
    ...
    def _on_double_click(self, filepath):
        ...
        self.after(100, lambda: self.on_play_callback() if self.on_play_callback else None)
    
    def _on_delete(self, filepath, filename):
        """Delete macro file"""
        if messagebox.askyesno("Xác nhận", f"Xóa macro '{filename.replace('.json', '')}'?"):
            try:
                os.remove(filepath)
...
class MacroTimelineCanvas(tk.Canvas):
    ...
    def __init__(self, parent, on_reorder_callback, on_edit_callback, **kwargs):
        ...
        # Drag state
        self.drag_data = {"item": None, "x": 0, "y": 0, "index": None, "start_x": 0, "start_y": 0, "dragging": False}
    ...
    def redraw(self):
        ...
        for i, item in enumerate(self.ui_items):
            if item['ui_type'] == 'delay':
                ...
                tag = f"item_{i}"
                # Background
                self.create_rectangle(x, y, x+w, y+self.item_height, 
                                     fill=colors['input_bg'], outline=colors['border'], 
                                     tags=(tag, "draggable"))
                # Text
                self.create_text(x + w/2, y + self.item_height/2, text=text, 
                ...
                # Shadow
                self.create_rectangle(x+2, y+2, x+w+2, y+self.item_height+2, 
                ...
                # Text
                self.create_text(x + (w-20)/2, y + self.item_height/2, text=details, 
                                fill="white", font=FONTS['bold'], tags=(tag, "draggable"))
                ...
                self.create_text(del_x, del_y, text="✕", fill="white", 
                                font=("Arial", 9, "bold"), tags=(tag, del_tag))
                
                # Bind delete specifically
                self.tag_bind(del_tag, "<Button-1>", lambda e, idx=i: self.delete_item(idx))
                # Hover effect for delete
                self.tag_bind(del_tag, "<Enter>", lambda e, t=del_tag: self.itemconfig(t, fill=colors['danger']))
                self.tag_bind(del_tag, "<Leave>", lambda e, t=del_tag: self.itemconfig(t, fill="white"))
    ...
    def _get_event_color(self, data):
        if data['type'] == 'key_press': return colors['primary']
        if data['type'] == 'mouse_click': return colors['accent']
        if data['type'] == 'mouse_scroll': return '#17a2b8'
    ...
    def _format_event_text(self, data):
        ...
        elif evt['type'] == 'key_press': return f"⌨ {d['key']} ↓"
        elif evt['type'] == 'key_release': return f"⌨ {d['key']} ↑"
        return str(d)
    ...
    def on_click(self, event):
        ...
        for t in tags:
            if t.startswith("del_"): return

        # Edit Delay Check
        ui_idx = -1
        for t in tags:
            if t.startswith("item_"):
                try: ui_idx = int(t.split("_")[1])
                except: pass
                break
        ...
        if idx is not None:
            ...
            if not self.drag_data["dragging"]:
                ...
                if ui_item and ui_item['ui_type'] == 'delay':
                    # Edit Delay
                    new_val = simpledialog.askfloat("Delay", "Enter delay (seconds):", 
                                                  initialvalue=ui_item['value'], parent=self)
            ...
            if target_idx is not None and target_idx != idx and 0 <= target_idx < len(self.ui_items):
                # Move in ui_items
...
class MacroRecorderFrame(tk.Frame):
    ...
    def setup_ui(self):
        ...
        self.btn_record = ModernButton(toolbar, text="🔴 Record (F9)", command=self.toggle_record,
                                   kind='danger', width=15)
        self.btn_record.pack(side='left', padx=(0, 10))
        
        self.btn_play = ModernButton(toolbar, text="▶ Play (Trigger)", command=self.toggle_play,
                                 kind='success', width=15)
        ...
        for text, val in modes:
            rb = ModernRadioButton(mode_frame, text=text, variable=self.playback_mode_var, value=val)
        ...
        tk.Label(t_header, text="Timeline (Drag to Reorder)", bg=colors['bg'], fg=colors['fg_dim'],
                font=FONTS['small']).pack(side='left')
        
        ModernButton(t_header, text="🗑 Clear All", command=self.clear_timeline,
                   kind='secondary', width=12).pack(side='right')
        
        ModernButton(t_header, text="⏳ + Delay", command=self.add_delay,
                   kind='accent', width=10).pack(side='right', padx=(0, 5))
        ...
        self.timeline = MacroTimelineCanvas(timeline_inner, 
        ...
        # Trigger Key
        tk.Label(settings_frame, text="Trigger:", bg=colors['bg'], fg=colors['fg'], 
                font=FONTS['body']).pack(side='left', padx=(0, 5))
        self.btn_trigger = ModernButton(settings_frame, text=self.trigger_key_var.get(), 
                                      command=self.start_set_trigger, width=15, kind='secondary')
        self.btn_trigger.pack(side='left', padx=5)
        
        # Save Button (Right side)
        ModernButton(settings_frame, text="💾 Save to Library", command=self.save_macro,
                 kind='primary').pack(side='right', padx=5)
        ...
        tk.Label(active_frame, text="Active Background Macros", 
                bg=colors['bg'], fg=colors['fg'], font=FONTS['h2']).pack(anchor='w', pady=(10, 5))
        ...
        ModernButton(ctrl_frame, text="+ Add Current to Active", 
                   command=self.add_current_to_active, kind='accent', width=20).pack(side='left')
        
        ModernButton(ctrl_frame, text="Clear All", 
                   command=self.clear_active_macros, kind='secondary', width=10).pack(side='right')

        # Status Bar
        self.status_var = tk.StringVar(value="Ready")
        tk.Label(self, textvariable=self.status_var, bg=colors['bg'], fg=colors['fg_dim'],
                font=FONTS['small']).pack(side='bottom', fill='x', pady=5)
    ...
    def show_menu(self):
        items = [
            {'label': 'Macro Library', 'command': self.show_macro_library, 'icon': '📚'},
    ...
    def update_edited_key(self, key):
        if self.editing_item_id is None: return
    ...
    def edit_delay(self, idx):
        ...
        new_delay = simpledialog.askfloat("Edit Delay", "Enter new delay (seconds):", 
                                           initialvalue=current_delay, parent=self)
    ...
    def stop_recording(self):
        ...
        self.refresh_list()
        self.status_var.set(f"Recorded {len(self.current_macro)} events (F9 auto-removed)")
    ...
    def play_macro(self):
        ...
        if mode == "loop":
            ...
            if fm.get_feature_limit("macro_infinite_loop"):
                ...
                count = simpledialog.askinteger("Loop Limit", "Gói hiện tại không hỗ trợ lặp vô hạn.\nNhập số lần lặp (1-10):", minvalue=1, maxvalue=10, parent=self)
                if count:
                    loop_count = count
                else:
                    self.stop_playback()
        ...
        self.player.play(self.current_macro, self.on_play_finished, loop_count=loop_count)
    ...
    def save_macro(self):
        ...
        if limit and isinstance(limit, int):
            ...
            if existing >= limit:
                messagebox.showwarning("Limit Reached", f"Gói hiện tại chỉ cho phép lưu tối đa {limit} macro.\nVui lòng nâng cấp để lưu thêm.")
                return
    ...
    def load_macro_from_path(self, filepath):
        ...
        if isinstance(data, list):
            ...
            if 'loop' in settings and settings['loop']: mode = 'loop'
            if 'hold' in settings and settings['hold']: mode = 'hold'
    ...
    def add_current_to_active(self):
        ...
        macro_name = self.current_macro_name if self.current_macro_name else f"Macro {len(self.active_macros) + 1}"
    ...
    def update_active_list(self):
        ...
        if not self.active_macros:
            tk.Label(self.active_container, text="No active macros", bg=colors['input_bg'], fg=colors['fg_dim']).pack(pady=10)
            return
    ...
    def _create_active_macro_card(self, trigger_code, data):
        ...
        tk.Label(card, text=icon, bg=colors['secondary'], fg='white', font=FONTS['h2']).pack(side='left', padx=5)
        ...
        # Name and Trigger
        tk.Label(info_frame, text=data['name'], bg=colors['secondary'], fg='white', font=FONTS['bold']).pack(anchor='w')
        tk.Label(info_frame, text=f"Trigger: {data['settings']['trigger']} • {len(data['events'])} events", 
                bg=colors['secondary'], fg=colors['fg_dim'], font=FONTS['small']).pack(anchor='w')
        
        # Remove Button
        ModernButton(card, text="✕", width=3, kind='danger', 
                   command=lambda t=trigger_code: self._remove_active_by_trigger(t)).pack(side='right', padx=5)
