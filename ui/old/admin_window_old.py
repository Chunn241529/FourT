...
class AdminWindow:
    ...
    def _setup_server_tab(self):
        ...
        # Status Section
        status_frame = tk.Frame(self.server_tab, bg=COLORS['card'], relief=tk.FLAT, bd=2)
    ...
    def _setup_packages_tab(self):
        ...
        main_container = tk.PanedWindow(self.packages_tab, orient=tk.HORIZONTAL, bg=COLORS['bg'],
        ...
        tk.Label(header, text="📦 Danh sách gói", font=FONTS['h2'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        
        ModernButton(header, text="🔄", command=self._load_packages,
        ...
        ModernButton(btn_frame, text="⬆", command=self._move_package_up,
                    kind='secondary', width=3).pack(side=tk.LEFT, padx=(0, 2))
        ModernButton(btn_frame, text="⬇", command=self._move_package_down,
                    kind='secondary', width=3).pack(side=tk.LEFT, padx=(0, 10))
        ModernButton(btn_frame, text="➕", command=self._add_package,
                    kind='success', width=3).pack(side=tk.LEFT, padx=(0, 2))
        ModernButton(btn_frame, text="❌", command=self._delete_package,
                    kind='danger', width=3).pack(side=tk.LEFT)
        ...
        tk.Label(right_panel, text="✏️ Chỉnh sửa gói", font=FONTS['h2'],
        ...
        self.pkg_form_canvas = tk.Canvas(scroll_container, bg=COLORS['card'], 
        ...
        def _on_pkg_scroll(event):
            if self.pkg_form_inner.winfo_reqheight() > self.pkg_form_canvas.winfo_height():
        ...
        def _on_inner_configure(event):
            self.pkg_form_canvas.configure(scrollregion=self.pkg_form_canvas.bbox("all"))
        ...
        tk.Label(row, text="ID:", width=12, anchor='w', font=FONTS['body'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        self.pkg_id_entry = tk.Entry(row, font=FONTS['body'], bg=COLORS['input_bg'],
        ...
        row = tk.Frame(form_frame, bg=COLORS['card'])
        row.pack(fill=tk.X, pady=5)
        tk.Label(row, text="Tên:", width=12, anchor='w', font=FONTS['body'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        self.pkg_name_entry = tk.Entry(row, font=FONTS['body'], bg=COLORS['input_bg'],
                                       fg=COLORS['fg'], insertbackground=COLORS['fg'])
        ...
        row = tk.Frame(form_frame, bg=COLORS['card'])
        row.pack(fill=tk.X, pady=5)
        tk.Label(row, text="Giá (VND):", width=12, anchor='w', font=FONTS['body'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        self.pkg_price_entry = tk.Entry(row, font=FONTS['body'], bg=COLORS['input_bg'],
                                        fg=COLORS['fg'], insertbackground=COLORS['fg'], width=15)
        self.pkg_price_entry.pack(side=tk.LEFT)
        
        # Duration (smart input)
        row = tk.Frame(form_frame, bg=COLORS['card'])
        row.pack(fill=tk.X, pady=5)
        tk.Label(row, text="Thời hạn:", width=12, anchor='w', font=FONTS['body'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        self.pkg_duration_entry = tk.Entry(row, font=FONTS['body'], bg=COLORS['input_bg'],
                                           fg=COLORS['fg'], insertbackground=COLORS['fg'], width=10)
        self.pkg_duration_entry.pack(side=tk.LEFT)
        tk.Label(row, text="(VD: 30, 1d, 7d, 1m, 1y, 30min)", font=FONTS['small'] if 'small' in FONTS else ('Segoe UI', 9),
                bg=COLORS['card'], fg=COLORS['fg_dim'] if 'fg_dim' in COLORS else COLORS['fg']).pack(side=tk.LEFT, padx=10)
        
        # Description
        row = tk.Frame(form_frame, bg=COLORS['card'])
        row.pack(fill=tk.X, pady=5)
        tk.Label(row, text="Mô tả:", width=12, anchor='w', font=FONTS['body'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        self.pkg_desc_entry = tk.Entry(row, font=FONTS['body'], bg=COLORS['input_bg'],
                                       fg=COLORS['fg'], insertbackground=COLORS['fg'])
        ...
        row = tk.Frame(form_frame, bg=COLORS['card'])
        row.pack(fill=tk.X, pady=5)
        tk.Label(row, text="Màu:", width=12, anchor='w', font=FONTS['body'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        self.pkg_color_entry = tk.Entry(row, font=FONTS['body'], bg=COLORS['input_bg'],
                                        fg=COLORS['fg'], insertbackground=COLORS['fg'], width=10)
        self.pkg_color_entry.pack(side=tk.LEFT)
        self.pkg_color_preview = tk.Label(row, text="  ██  ", bg=COLORS['accent'], width=5)
        ...
        tk.Checkbutton(row, text="Gói khuyến nghị (Recommended)", variable=self.pkg_recommended_var,
        ...
        features_frame = tk.LabelFrame(form_frame, text="Tính năng (Feature IDs)", font=FONTS['bold'],
                                       bg=COLORS['card'], fg=COLORS['fg'], padx=10, pady=10)
        ...
        for i, (feature_id, feature_name) in enumerate(self.all_features):
            ...
            col = i % 2
            row_idx = i // 2
            cb = tk.Checkbutton(features_inner, text=feature_name, variable=var,
                               bg=COLORS['card'], fg=COLORS['fg'], selectcolor=COLORS['input_bg'],
                               activebackground=COLORS['card'], activeforeground=COLORS['fg'],
                               font=FONTS['body'])
        ...
        display_frame = tk.LabelFrame(form_frame, text="Mô tả hiển thị (1 dòng = 1 tính năng)", font=FONTS['bold'],
                                      bg=COLORS['card'], fg=COLORS['fg'], padx=10, pady=10)
        display_frame.pack(fill=tk.X, pady=10)
        
        tk.Label(display_frame, text="Nhập mô tả cho client (mỗi dòng 1 tính năng):", font=FONTS['small'] if 'small' in FONTS else ('Segoe UI', 9),
                bg=COLORS['card'], fg=COLORS['fg_dim'] if 'fg_dim' in COLORS else COLORS['fg']).pack(anchor='w')
        
        self.pkg_feature_display = tk.Text(display_frame, font=FONTS['body'], bg=COLORS['input_bg'],
                                           fg=COLORS['fg'], insertbackground=COLORS['fg'], height=5,
                                           width=40)
        self.pkg_feature_display.pack(fill=tk.X, pady=5)
        
        # Limits section
        limits_frame = tk.LabelFrame(form_frame, text="Giới hạn", font=FONTS['bold'],
                                     bg=COLORS['card'], fg=COLORS['fg'], padx=10, pady=10)
        limits_frame.pack(fill=tk.X, pady=5)
        
        row = tk.Frame(limits_frame, bg=COLORS['card'])
        row.pack(fill=tk.X, pady=3)
        tk.Label(row, text="Macro save limit:", font=FONTS['body'],
                bg=COLORS['card'], fg=COLORS['fg']).pack(side=tk.LEFT)
        self.pkg_macro_limit_entry = tk.Entry(row, font=FONTS['body'], bg=COLORS['input_bg'],
                                              fg=COLORS['fg'], width=8)
        self.pkg_macro_limit_entry.pack(side=tk.LEFT, padx=5)
        
        self.pkg_infinite_loop_var = tk.BooleanVar(value=False)
        tk.Checkbutton(row, text="Infinite Loop", variable=self.pkg_infinite_loop_var,
                      bg=COLORS['card'], fg=COLORS['fg'], selectcolor=COLORS['input_bg'],
                      font=FONTS['body']).pack(side=tk.LEFT, padx=15)
        
        # Save button
        save_frame = tk.Frame(right_panel, bg=COLORS['card'])
        save_frame.pack(fill=tk.X, padx=15, pady=15)
        
        ModernButton(save_frame, text="💾 Lưu gói", command=self._save_package,
    ...
    def _load_packages(self):
        ...
        try:
            ...
            from core.config import PACKAGE_FEATURES, PACKAGE_PRICES, PACKAGE_DETAILS, Packages
            self.packages_data = {}
            for pkg_id in [Packages.FREE, Packages.BASIC, Packages.PLUS, Packages.PRO, Packages.PREMIUM]:
                ...
                self.packages_data[pkg_id] = {
                    ...
                    "features": [f.split('.')[-1] if '.' in str(f) else str(f) for f in PACKAGE_FEATURES.get(pkg_id, [])],
                    "color": details.get("color", "#95a5a6"),
                    "recommended": details.get("recommended", False)
                }
        
        # Populate listbox
        for pkg_id in sorted(self.packages_data.keys(), key=lambda x: self.packages_data[x].get('order', 99)):
    ...
    def _on_package_select(self, event):
        ...
        if pkg.get('duration_minutes'):
            ...
            self.pkg_duration_entry.insert(0, self._format_duration_display(pkg.get('duration_days', 30)))
    ...
    def _save_package(self):
        ...
        feature_display = [line.strip() for line in feature_display_text.split('\n') if line.strip()]
        ...
        if pkg_id in self.packages_data:
            ...
            order = max([p.get('order', 0) for p in self.packages_data.values()], default=0) + 1
        ...
        try:
            ...
            messagebox.showwarning("Lưu Local", f"Server không khả dụng.\nDữ liệu chỉ lưu local (cần restart server).")
            # Update local data
            self.packages_data[pkg_id] = data
            self._load_packages()
        except Exception as e:
            messagebox.showwarning("Lưu Local", f"{e}\n\nDữ liệu chỉ lưu local (cần restart server).")
            # Update local data
            self.packages_data[pkg_id] = data
            self._load_packages()
    ...
    def _move_package_up(self):
        ...
        pkg_ids = [self.packages_listbox.get(i).split(':')[0] for i in range(self.packages_listbox.size())]
    ...
    def _save_order_to_server(self, pkg_id):
        ...
        try:
            ...
            requests.post(f"{admin_api_url}/features/admin/packages/{pkg_id}", json=data, timeout=5)
        except:
            pass
    ...
    def _setup_skills_tab(self):
        ...
        main_container = tk.PanedWindow(self.skills_tab, orient=tk.HORIZONTAL, bg=COLORS['bg'], 
                                         sashwidth=6, sashrelief=tk.RAISED)
        main_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        tk.Label(filter_frame, text="Lọc:", font=FONTS['small'], 
        ...
        tk.Label(row, text="(lmb/rmb/mouse4/scroll_down...)", font=FONTS['small'], 
                bg=COLORS['card'], fg=COLORS['text_dim'] if 'text_dim' in COLORS else COLORS['fg']).pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(row, text="Dùng [image: file.gif] chèn ảnh. *In đậm*, [#ff0000:Màu đỏ], [khaki:Màu Khaki]", font=FONTS['small'], 
                bg=COLORS['card'], fg=COLORS['text_dim'] if 'text_dim' in COLORS else COLORS['fg']).pack(side=tk.TOP, anchor='w', pady=(0, 5))
        ...
        tk.Label(menu_preview_frame, text="📋 Menu Skill", font=FONTS['small'], 
                bg=COLORS['card'], fg=COLORS['fg_dim'] if 'fg_dim' in COLORS else COLORS['fg']).pack(anchor=tk.W, padx=5, pady=5)
        ...
        self.menu_preview_key = tk.Label(menu_inner, text="[KEY]", font=FONTS['small'], 
                                         bg=COLORS['bg'], fg=COLORS['fg_dim'] if 'fg_dim' in COLORS else COLORS['fg'])
        ...
        tk.Label(timeline_preview_frame, text="� Timeline", font=FONTS['small'], 
                bg=COLORS['card'], fg=COLORS['fg_dim'] if 'fg_dim' in COLORS else COLORS['fg']).pack(anchor=tk.W, padx=5, pady=5)
    ...
    def _refresh_weapons_list(self):
        ...
        for idx, weapon in enumerate(weapons):
            ...
            is_selected = (hasattr(self, 'selected_weapon_idx') and 
            ...
            self.weapons_canvas.create_rectangle(
                x1, y1, x2, y2,
                fill=bg_color, outline=COLORS['border'] if is_hovered else '',
        ...
        if abs(event.y - self.weapon_drag_data["start_y"]) > 10:
            ...
            if target_idx is not None and target_idx != self.weapon_drag_data["idx"]:
                ...
                if 0 <= src_idx < len(weapons) and 0 <= target_idx < len(weapons):
                    ...
                    self.weapon_drag_data["start_y"] = event.y  # Reset start_y to avoid repeated swaps
    ...
    def _add_license(self):
        ...
        header.pack_propagate(False)
        tk.Label(header, text="➕ Create New License", font=FONTS['h2'],
                bg=COLORS['header'], fg=COLORS['fg']).pack(side=tk.LEFT, padx=15, pady=12)
        ...
        tk.Label(row, text="License Key:", font=FONTS['body'], width=12, anchor='w',
                bg=COLORS['bg'], fg=COLORS['fg']).pack(side=tk.LEFT)
        
        key_var = tk.StringVar()
        key_entry = tk.Entry(row, textvariable=key_var, font=FONTS['body'],
                            bg=COLORS['input_bg'], fg=COLORS['fg'],
                            insertbackground=COLORS['fg'], width=25)
        ...
        ModernButton(row, text="🔄", command=generate_key, kind='secondary', width=3).pack(side=tk.LEFT, padx=2)
        
        # Package
        row = tk.Frame(content, bg=COLORS['bg'])
        row.pack(fill=tk.X, pady=8)
        tk.Label(row, text="Package:", font=FONTS['body'], width=12, anchor='w',
                bg=COLORS['bg'], fg=COLORS['fg']).pack(side=tk.LEFT)
        
        package_var = tk.StringVar(value="premium")
        package_combo = ttk.Combobox(row, textvariable=package_var, state='readonly', width=22,
        ...
        row = tk.Frame(content, bg=COLORS['bg'])
        row.pack(fill=tk.X, pady=8)
        tk.Label(row, text="Duration:", font=FONTS['body'], width=12, anchor='w',
                bg=COLORS['bg'], fg=COLORS['fg']).pack(side=tk.LEFT)
        
        duration_options = [("7 days", 7), ("30 days", 30), ("90 days", 90), ("365 days", 365)]
        duration_var = tk.StringVar(value="30 days")
        duration_combo = ttk.Combobox(row, textvariable=duration_var, state='readonly', width=22,
        ...
        row = tk.Frame(content, bg=COLORS['bg'])
        row.pack(fill=tk.X, pady=8)
        tk.Label(row, text="Notes:", font=FONTS['body'], width=12, anchor='w',
                bg=COLORS['bg'], fg=COLORS['fg']).pack(side=tk.LEFT)
        
        notes_var = tk.StringVar()
        notes_entry = tk.Entry(row, textvariable=notes_var, font=FONTS['body'],
                              bg=COLORS['input_bg'], fg=COLORS['fg'],
                              insertbackground=COLORS['fg'], width=28)
        ...
        def create_license():
            ...
            if not key:
                messagebox.showwarning("Missing Key", "Please enter or generate a license key.")
                return
            ...
            if self.license_service.add_license(key, package, days, notes):
                ...
                messagebox.showerror("Error", "Failed to create license. Key may already exist.")
        
        ModernButton(btn_frame, text="Create", command=create_license,
                    kind='success', width=12).pack(side=tk.RIGHT, padx=5)
        ModernButton(btn_frame, text="Cancel", command=dialog.destroy,
                    kind='secondary', width=12).pack(side=tk.RIGHT)
    ...
    def _kill_existing_processes(self):
        ...
        killed_processes = self.server_service.kill_existing_processes(self._log_to_terminal)
    ...
    def _start_server(self):
        ...
        try:
            ...
            if killed_count > 0:
                self._log_to_terminal(f"Cleaned up {killed_count} existing process(es)\n\n")
            else:
                self._log_to_terminal("No existing processes found\n\n")
            
            # Start server using service
            self.server_process = self.server_service.start_server(self._log_to_terminal)
            ...
            try:
                ...
                if dist_path.exists():
                    ...
                    try:
                        ...
                        self._log_build(f"⚠️ Warning: Could not delete dist folder: {e}\n")
                
                script_path = self.backend_dir / "build_nuitka.ps1"
                if not script_path.exists():
                    self._log_build(f"ERROR: build_nuitka.ps1 not found at {script_path}")
                ...
                process = subprocess.Popen(
                    ["powershell", "-ExecutionPolicy", "Bypass", "-File", str(script_path), "-NewVersion", new_version],
                ...
                if process.returncode == 0:
                    ...
                    self.root.after(0, lambda: self.current_version_label.config(text=new_version))
                    self.root.after(0, lambda: self.new_version_entry.delete(0, tk.END))
                    self.root.after(0, lambda: self.new_version_entry.insert(0, self._get_next_version()))
                else:
    ...
    def _build_installer(self):
        ...
        def run_build():
            try:
                ...
                if not script_path.exists():
                    self._log_build(f"ERROR: build_installer.ps1 not found at {script_path}")
                    self._update_build_status("Build failed!", False)
                    return
    ...
    def _show_download_link(self, download_url: str):
        ...
        self.download_frame.pack(fill=tk.X, padx=20, pady=(0, 10), after=self.new_version_entry.master.master)
    ...
    def _copy_download_url(self):
        ...
        if hasattr(self, 'download_url') and self.download_url:
            ...
            messagebox.showinfo("Copied", f"Download URL copied to clipboard!\n\n{self.download_url}")
