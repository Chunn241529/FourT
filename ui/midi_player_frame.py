...
class MidiPlayerFrame(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        # Co-op Mode
        self.coop_btn = ToggleButton(
            mode_frame,
            "👥",
            command=self._open_coop_window,
            active=False,
            width=42,
            height=36,
        )
        self.coop_btn.pack(side="left", padx=2)

        # Hand Mode Toggle (Co-op)
        # 0: 👐 (Both), 1: 🤚L (Left/Bass), 2: 🤚R (Right/Melody)
        self.hand_mode = 0
        self.hand_btn = ToggleButton(
            mode_frame,
            "👐",
        ...
        self.hand_btn.pack(side="left", padx=2)

        # Humanize Toggle (Anti-cheat)
        self.humanize_btn = ToggleButton(
            mode_frame,
            "🧬",
            command=self._toggle_humanize,
    ...
    def _toggle_hand_mode(self):
        """Toggle Hand Mode: Both -> Left -> Right"""
        self.hand_mode = (self.hand_mode + 1) % 3

        mode_text = {
            0: ("👐", "Cả 2 tay", False),
            1: ("🤚L", "Tay Trái (Bass)", True),
            2: ("🤚R", "Tay Phải (Melody)", True),
        }

        icon, desc, active = mode_text[self.hand_mode]

        self.hand_btn.set_text(icon)
        self.hand_btn.set_active(active)
        self.playback_engine.set_hand_mode(self.hand_mode)

        self.status_label.config(text=f"Chế độ: {desc}", fg=colors["accent"])

    def _toggle_humanize(self):
        """Toggle humanization (anti-cheat)"""
        new_state = not self.playback_engine.humanize_enabled
        self.playback_engine.set_humanize(new_state)
        self.humanize_btn.set_active(new_state)

        if not new_state:
            # Warning when disabling
            messagebox.showwarning(
                "Cảnh Báo Anti-Cheat",
                "Tắt chế độ Humanize sẽ khiến nhạc phát đều như robot (100% chuẩn).\n\n"
                "Điều này RẤT NGUY HIỂM và dễ bị game phát hiện là macro.\n"
                "Bạn chỉ nên tắt nếu test offline hoặc chấp nhận rủi ro!",
            )
        else:
    ...
    def _add_from_file(self):
        ...
        count = 0
        for path in paths:
            if self.playlist.add_song(path) == -1:
                full = True
                break
            count += 1

        if full:
            messagebox.showwarning(t("limit_reached"), t("playlist_limit_msg"))
        elif count > 0:
            self.status_label.config(
                text=t("songs_added", count=count), fg=colors["success"]
            )
    ...
    def _on_song_remove(self, index):
        ...
        current_idx = self.playlist.get_current_index()
        if index == current_idx and self.playback_engine.is_active():
            self.playback_engine.stop()
            self.play_btn.set_text("▶")
    ...
    def _generate_script(self):
        ...
        fm = get_feature_manager()
        if not fm.has_feature(Features.SCRIPT_PREVIEW):
            messagebox.showinfo(t("upgrade_required"), t("feature_not_available_basic"))
            return
    ...
    def _open_coop_window(self):
        """Open Co-op Window"""
        from .coop_window import CoopWindow

        if hasattr(self, "coop_window") and self.coop_window.winfo_exists():
            try:
                self.coop_window.lift()
                self.coop_window.focus_force()
            except:
                self.coop_window = CoopWindow(
                    self, on_start_playback=self._play_current
                )
            return

        self.coop_window = CoopWindow(self, on_start_playback=self._play_current)
