...
class SongListItem(tk.Frame):
    ...
    def __init__(self, parent, index, song, is_current=False, on_click=None, on_remove=None, **kwargs):
        ...
        self.play_icon = tk.Label(
            content, text="▶" if is_current else "  ",
            font=('Segoe UI', 8),
            bg=colors['card'],
            fg=colors['accent'],
            width=2
    ...
    def _on_enter(self, e):
        ...
        for w in [self.indicator, self.index_label, self.play_icon, self.name_label]:
            try: w.configure(bg=colors['card_hover'])
            except: pass
    ...
    def _on_leave(self, e):
        ...
        for w in [self.indicator, self.index_label, self.play_icon, self.name_label, self.remove_btn]:
    ...
    def _on_click(self, e):
        if self.on_click: self.on_click(self.index)
    
    def _on_double_click(self, e):
        if self.on_click: self.on_click(self.index, play=True)
    
    def _on_remove_click(self, e):
        if self.on_remove: self.on_remove(self.index)
...
class ToggleButton(tk.Canvas):
    ...
    def _draw(self):
        ...
        if self._active:
            fill, text_color = colors['accent'], 'white'
        elif self._hover:
        ...
        self.create_text(w/2, h/2, text=self.text, fill=text_color, font=('Segoe UI Emoji', 11))
    ...
    def set_active(self, active): self._active = active; self._draw()
    def set_text(self, text): self.text = text; self._draw()


class MidiPlayerFrame(tk.Frame):
    """Main MIDI Player UI - Merged with Playlist functionality"""
    
    def __init__(self, parent, playback_engine, library_service, update_service, **kwargs):
        super().__init__(parent, **kwargs)
    ...
    def _create_widgets(self):
        ...
        tk.Label(title_frame, text="🎹", font=('Segoe UI Emoji', 18),
                 bg=colors['bg'], fg=colors['accent']).pack(side='left')
        tk.Label(title_frame, text="MIDI Auto Player", font=('Segoe UI', 16, 'bold'),
                 bg=colors['bg'], fg=colors['fg']).pack(side='left', padx=(8, 0))
        
        # Add button
        self.add_btn = ModernButton(header, text="+ Thêm", command=self._show_add_menu,
                                     kind='accent', width=8)
        ...
        self.canvas = tk.Canvas(list_container.content, bg=colors['card'],
                                 highlightthickness=0, bd=0)
        self.scrollbar = ttk.Scrollbar(list_container.content, orient='vertical',
        ...
        # Separator
        tk.Frame(playback_frame, width=1, bg=colors['border']).pack(side='left', fill='y', padx=8)
        
        # Preview button
        self.preview_btn = ToggleButton(playback_frame, "👁️", command=self._generate_script, width=42, height=36)
        self.preview_btn.pack(side='left', padx=2)
        
        # Speed label (clickable)
        self.current_speed = DEFAULT_PLAYBACK_SPEED
        self.speed_label = tk.Label(
            playback_frame, text=f"{self.current_speed:.1f}x",
        ...
        self.shuffle_btn = ToggleButton(mode_frame, "🔀", command=self._toggle_shuffle,
                                         active=self.playlist.shuffle_enabled, width=42, height=36)
        self.shuffle_btn.pack(side='left', padx=2)
        
        self.repeat_btn = ToggleButton(mode_frame, "🔁", command=self._toggle_repeat,
        ...
        self.count_label = tk.Label(footer, text="0 bài", font=('Segoe UI', 9),
                                     bg=colors['bg'], fg=colors['fg_dim'])
        ...
        ModernButton(actions, text="💾", command=self._save_playlist,
        ...
        self.stop_all_btn = ModernButton(actions, text="⏹", command=self._stop_all,
                     kind='danger', width=4, font=('Segoe UI Emoji', 11))
    ...
    def _refresh_list(self):
        ...
        if not songs:
            tk.Label(self.songs_frame, text="\n🎵\n\nPlaylist trống\n\nClick '+ Thêm' để thêm bài",
                     font=('Segoe UI', 11), bg=colors['card'], fg=colors['fg_dim'],
                     justify='center', anchor='center').pack(fill='both', expand=True)
        else:
            is_playing = self.playback_engine.is_active()
            for i, song in enumerate(songs):
                SongListItem(self.songs_frame, index=i, song=song,
                             is_current=(i == current_idx),
                             on_click=self._on_song_click,
                             on_remove=self._on_song_remove if not is_playing else None).pack(fill='x', pady=1)
        ...
        if count > 0 and current_idx >= 0:
            ...
            if len(name) > 20:
                name = name[:17] + "..."
            self.count_label.config(text=f"{count} bài • {name}")
        else:
    ...
    def _stop_all(self):
        ...
        self.status_label.config(text="⏹ Đã dừng tất cả", fg=colors['warning'])
        self._refresh_list()
    ...
    def _show_add_menu(self):
        ...
        menu.add_separator()
        menu.add_command(label="🎵  MP3 → MIDI...", command=self._convert_mp3_to_midi)
    ...
    def _add_from_library(self):
        ...
        tk.Label(dialog, text="📂 Chọn bài hát", font=('Segoe UI', 14, 'bold'),
                 bg=colors['bg'], fg=colors['fg']).pack(padx=15, pady=15, anchor='w')
        
        list_frame = tk.Frame(dialog, bg=colors['card'])
        list_frame.pack(fill='both', expand=True, padx=15, pady=5)
        
        listbox = tk.Listbox(list_frame, selectmode='extended', bg=colors['card'],
                             fg=colors['fg'], font=('Segoe UI', 10),
        ...
        ModernButton(btn_frame, text="Thêm", command=add_selected, kind='accent', width=10).pack(side='left')
    ...
    def _add_from_file(self):
        paths = filedialog.askopenfilenames(filetypes=[("MIDI files", "*.mid *.midi")], title="Chọn file MIDI")
        for path in paths:
    ...
    def _convert_mp3_to_midi(self):
        ...
        if not fm.has_feature(Features.MP3_CONVERSION):
            messagebox.showinfo("Nâng cấp", "Tính năng MP3→MIDI từ gói Pro.")
            return
        ...
        self.status_label.config(text="⏳ Đang chuyển đổi...", fg=colors['accent'])
        self.update()
        ...
        def convert_thread():
            try:
                ...
                if is_server_offline():
                    self.after(0, lambda: self.status_label.config(text="❌ Server offline", fg=colors['error']))
                    return
                
                server_url = get_license_server_url()
                with open(path, 'rb') as f:
                    files = {'file': (os.path.basename(path), f)}
                    response = requests.post(f"{server_url}/midi/convert", files=files, timeout=300)
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success'):
                        ...
                        if midi_response.status_code == 200:
                            local_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "midi_files")
                            ...
                            def on_success():
                                self.status_label.config(text=f"✅ Đã tạo: {midi_filename}", fg=colors['success'])
                ...
                self.after(0, lambda: self.status_label.config(text=f"❌ Lỗi: {str(e)[:30]}", fg=colors['error']))
    ...
    def _play_current(self):
        ...
        if not fm.has_feature(Features.MIDI_PLAYBACK):
            messagebox.showinfo("Hết hạn", "Vui lòng nâng cấp để tiếp tục.")
            return
        ...
        if self.playback_engine.is_active():
            self.playback_engine.stop()
            self.play_btn.set_text("▶")
            self.status_label.config(text="⏸️ Đã dừng", fg=colors['warning'])
            return
        ...
        # Process MIDI
        self.status_label.config(text="⏳ Đang xử lý...", fg=colors['accent'])
        self.update()
        ...
        try:
            ...
            if self.generated_events and len(self.generated_events) > 0:
                ...
                events, total_time, debug_info = preprocess_midi(path, auto_transpose=self.auto_transpose)
                self.generated_events = events
                self.last_debug_info = debug_info
        except Exception as e:
            messagebox.showerror("Lỗi", f"Lỗi đọc MIDI: {e}")
            self.status_label.config(text="❌ Lỗi!", fg=colors['error'])
            return
        
        if len(events) == 0:
            messagebox.showwarning("Cảnh báo", "File MIDI trống!")
            return
    ...
    def _countdown(self, count, events):
        if count > 0:
            self.status_label.config(text=f"Chuyển sang game trong... {count} giây", fg=colors['warning'])
    ...
    def _begin_playback(self, events):
        self.play_btn.set_text("⏹")
        self.status_label.config(text="🎵 Đang phát...", fg=colors['success'])
        
        def on_complete():
            self.play_btn.set_text("▶")
            self.status_label.config(text="✅ Hoàn thành", fg=colors['success'])
            ...
            # Auto advance
            if self.playlist.repeat_mode != PlaylistService.REPEAT_NONE or self.playlist.has_next():
    ...
    def _generate_script(self):
        song = self.playlist.get_current_song()
        if not song:
            messagebox.showinfo("Thông báo", "Chọn một bài để xem trước!")
            return
        ...
        try:
            ...
            if self.script_viewer_window and tk.Toplevel.winfo_exists(self.script_viewer_window):
                self.script_viewer_window.lift()
                self.script_viewer_window.focus_force()
                self.status_label.config(text="✅ Xem trước", fg=colors['success'])
                return
            
            # Use existing events if available, otherwise process
            if self.generated_events and hasattr(self, 'last_debug_info') and self.last_debug_info:
                events = self.generated_events
                debug_info = self.last_debug_info
            else:
                events, _, debug_info = preprocess_midi(path, auto_transpose=self.auto_transpose)
                self.generated_events = events
                self.last_debug_info = debug_info
    ...
    def _save_playlist(self):
        if self.playlist.is_empty():
            ...
            return
        name = simpledialog.askstring("Lưu Playlist", "Nhập tên:", parent=self)
        if name:
    ...
    def _load_playlist(self):
        ...
        if not playlists:
            messagebox.showinfo("Thông báo", "Chưa có playlist")
            return
        ...
        tk.Label(dialog, text="📂 Chọn playlist", font=('Segoe UI', 14, 'bold'),
                 bg=colors['bg'], fg=colors['fg']).pack(padx=15, pady=15, anchor='w')
        
        listbox = tk.Listbox(dialog, bg=colors['card'], fg=colors['fg'],
    ...
    def _clear_playlist(self):
        if self.playlist.is_empty():
            return
        if messagebox.askyesno("Xác nhận", "Xóa tất cả?"):
            self.playlist.clear()
