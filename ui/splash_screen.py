...
class ProgressManager:
    ...
    def __init__(self, task_ids: list):
        ...
        self.total_weight = sum(self.TASKS[tid]["weight"] for tid in task_ids if tid in self.TASKS)
    ...
    def complete_task(self, task_id: str) -> float:
        ...
        self._current_percent = (self.completed_weight / self.total_weight * 100) if self.total_weight > 0 else 100
...
class SplashScreen:
    ...
    def _create_ui(self):
        ...
        self._draw_rounded_rect(
            ...
            self.progress_x2, self.progress_y + self.progress_height,
            radius=2,
            fill='#1a1a2e',
            outline=''
        )
        ...
        self.progress_bar_id = self._draw_rounded_rect(
            self.progress_x1, self.progress_y,
            self.progress_x1 + 1, self.progress_y + self.progress_height,
            radius=2,
            fill='#667eea',
            outline=''
        )
    ...
    def _update_progress_bar(self, percent):
        ...
        self.progress_bar_id = self._draw_rounded_rect(
            self.progress_x1, self.progress_y,
            self.progress_x1 + fill_width, self.progress_y + self.progress_height,
            radius=2,
            fill='#667eea',
            outline=''
        )
    ...
    def _run_animation(self):
        ...
        try:
            if abs(self._current_display_percent - self._target_percent) < 0.5:
                ...
                self.canvas.itemconfig(self.percent_id, text=f"{int(self._current_display_percent)}%")
                self._animation_id = None
                return
            ...
            # Schedule next frame
            self._animation_id = self.root.after(self.ANIMATION_INTERVAL, self._run_animation)
        finally:
            self._animating = False
            ...
            for i, color in enumerate(alpha_colors):
                ...
                if sx1 < self.progress_x2:
                    self.shimmer_id = self.canvas.create_rectangle(
                        sx1, self.progress_y,
                        sx2, self.progress_y + self.progress_height,
                        fill=color, outline=''
                    )
    ...
    def _run_tasks(self):
        ...
        try:
            ...
            if is_online:
                ...
                full_task_list = [
                    "connect", "url_refresh", "update", "download",
                    "cache", "memory", "license", "skills", 
                    "modules", "icons", "templates", "midi", "ready"
                ]
                ...
                offline_tasks = ["connect", "cache", "memory", "license", "modules", "ready"]
    ...
    def _run_sync(self):
        ...
        try:
            ...
            for task_id in ["license", "skills", "modules", "icons", "templates", "midi"]:
        ...
        def download_thread():
            try:
                ...
                def on_progress(current, total, percent):
                    ...
                    self._update_ui(f"Downloading... {current:.1f}MB / {total:.1f}MB", p)
                ...
                if success:
                    ...
                    def show_dialog():
                        ...
                        def on_later():
                            ...
                            print("[Splash] User chose to update later, continuing to app")
                            self._complete()
