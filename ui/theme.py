...
class ModernRadioButton(tk.Canvas):
    ...
    def __init__(
        ...
        self._hover = False
        self._animation_id = None
        self._check_scale = 1.0 if self._selected else 0.0
    ...
    def _draw(self):
        ...
        # Circle background
        border = self.hover_border if self._hover else self.border_color
        fill = self.box_color  # Radio buttons usually rely on the inner dot

        # Draw outer circle
        self.create_oval(x, y, x + s, y + s, fill=fill, outline=border, width=2)

        # Draw inner dot with animation scale
        if self._check_scale > 0:
            scale = self._check_scale
            # Max dot radius is about s/2 - 4
            max_r = (s / 2) - 4
            curr_r = max_r * scale
    ...
    def _on_click(self, event):
        ...
        if self.variable and self.value:
            ...
            # Animation triggered by var change
...
class RoundedCard(tk.Canvas):
    """Container with rounded corners and optional gradient/solid background"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            height=height,
            bg=kwargs.pop(
                "bg", parent["bg"] if "bg" in parent.keys() else COLORS["bg"]
            ),
            highlightthickness=0,
            **kwargs,
        )
        ...
        self.content_window = self.create_window(
            width / 2, height / 2, window=self.content, anchor="center"
        )
    ...
    def _on_resize(self, event):
        ...
        self.coords(self.content_window, w / 2, h / 2)
        # Resize content frame with padding
        current_pad = self.radius // 2
        self.itemconfigure(
            self.content_window, width=w - self.radius, height=h - self.radius
        )
        self._draw_bg(w, h)

    def _draw_bg(self, w=None, h=None):
        ...
        # Simplification: Poly logic. Better use proper arcs for smoothness, but poly is okay for now.
        # Actually reusing RoundedButton logic is better for quality.

        # Let's use clean arc drawing
        self.create_arc(
            0,
            0,
            2 * r,
        ...
        self.create_rectangle(
            r, 0, w - r, h + 1, fill=self.bg_color, outline="", tags="bg_shape"
        )
        self.create_rectangle(
            0, r, w + 1, h - r, fill=self.bg_color, outline="", tags="bg_shape"
        )
...
class LoadingSpinner(tk.Canvas):
    ...
    def __init__(self, parent, size=40, color=None, bg=None, **kwargs):
        self.size = size
