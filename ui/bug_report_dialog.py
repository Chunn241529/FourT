...
class BugReportDialog:
    ...
    def __init__(self, parent):
        ...
        self.dialog = FramelessWindow(parent, title=t("bug_report"), icon_path=None)
    ...
    def _setup_ui(self):
        ...
        self.browse_btn = ModernButton(
            file_row, text=t("choose_file"), command=self._browse_file, kind="secondary"
        )
        self.browse_btn.pack(side="right", padx=(10, 0), ipady=4)

        # Clear file button (hidden initially)
        self.clear_btn = ModernButton(
            file_row, text="✕", command=self._clear_file, kind="danger", width=3
        )
        ...
        self.submit_btn = ModernButton(
            btn_frame, text=t("send_report"), command=self._submit, kind="success"
        )
    ...
    def _browse_file(self):
        ...
        filetypes = [
            (
                "Hình ảnh & Video",
                "*.png *.jpg *.jpeg *.gif *.bmp *.webp *.mp4 *.webm *.mov *.avi *.mkv",
            ),
        ...
        filepath = filedialog.askopenfilename(
            parent=self.dialog, title="Chọn hình ảnh hoặc video", filetypes=filetypes
        )
    ...
    def _validate_and_set_file(self, filepath):
        ...
        try:
            ...
            all_allowed = (
                self.ALLOWED_EXTENSIONS["images"] + self.ALLOWED_EXTENSIONS["videos"]
            )
            ...
            # Update UI
            self.file_label.config(text=filename, fg=colors["fg"])
            self.file_info_label.config(
                text=f"✓ {file_size_mb:.1f}MB - Đã chọn", fg=colors["success"]
            )
            ...
            messagebox.showerror("Lỗi", f"Không thể đọc file: {e}", parent=self.dialog)

    def _clear_file(self):
        """Clear attached file"""
        self.attached_file = None
        self.file_label.config(text=t("no_file"), fg=colors["text_dim"])
    ...
    def _submit(self):
        ...
        if not title:
            self.status_label.config(text=t("enter_title"), fg=colors["warning"])
            self.title_entry.focus_set()
            return

        if not desc:
            self.status_label.config(text=t("enter_description"), fg=colors["warning"])
            self.desc_text.focus_set()
            return

        # Start submission
        self.submitting = True
        self.submit_btn.config(state="disabled", text=t("sending"))
        self.status_label.config(text=t("sending_report"), fg=colors["accent"])

        # Submit in background
        threading.Thread(
            target=self._do_submit, args=(title, desc, self.attached_file), daemon=True
        ).start()
    ...
    def _fallback_to_email(self, title, desc, filepath):
        ...
        mailto_url = f"mailto:{email}?subject={subject}&body={body}".replace(
            " ", "%20"
        ).replace("\n", "%0A")
    ...
    def _on_submit_error(self, error):
        ...
        def update():
            ...
            self.status_label.config(
                text=f"❌ Lỗi: {error[:50]}...", fg=colors["error"]
            )
...
class ThankYouPopup:
    ...
    def __init__(self, parent, via_email=False):
        ...
        # Create popup window
        self.popup = FramelessWindow(parent, title="Cảm ơn bạn!", icon_path=None)
    ...
    def _setup_ui(self):
        ...
        if self.via_email:
            message = t("email_opened")
        else:
            message = t("report_sent")
        ...
        close_btn = ModernButton(
            container, text=t("close"), command=self._close, kind="secondary"
        )
