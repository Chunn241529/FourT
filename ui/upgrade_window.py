...
class UpgradeWindow:
    ...
    def _on_fetch_error(self):
        """Handle fetch error - use local fallback and show offline message"""
        from core.config import PACKAGE_DETAILS, PACKAGE_PRICES, PACKAGE_FEATURES

        self.is_offline = True  # Flag for UI to show offline message
    ...
    def _create_widgets(self):
        ...
        # Show offline warning if server unreachable
        if getattr(self, "is_offline", False):
            offline_banner = tk.Frame(
                content,
                bg="#3a2a1a",
                highlightbackground="#ff9800",
                highlightthickness=1,
            )
            offline_banner.pack(fill="x", padx=25, pady=(10, 5))
            tk.Label(
                offline_banner,
                text=t("server_offline_msg"),
                font=("Segoe UI", 11),
                bg="#3a2a1a",
                fg="#ffcc80",
                pady=10,
    ...
    def _create_card(self, parent, package_id, pkg_data, col_index):
        ...
        if duration_days == 30:
            duration_text = t("month_1")
        elif duration_days == 365:
            duration_text = t("year_1")
        elif duration_days == 7:
            duration_text = t("week_1")
        else:
            duration_text = t("days", days=duration_days)
