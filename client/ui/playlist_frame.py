...
class ToggleButton(tk.Canvas):
    ...
    def _draw(self):
        ...
        # Text
        self.create_text(
            w / 2, h / 2, text=self.text, fill=text_color, font=("Segoe UI Emoji", 12)
        )
...
class PlaylistFrame(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.add_btn = ModernButton(
            header, text="+ Thêm", command=self._show_add_menu, kind="accent", width=8
        )
        ...
        self.play_btn = ToggleButton(
            playback_frame, "▶", command=self._play_current, width=42, height=36
        )
    ...
    def _add_from_library(self):
        ...
        ModernButton(
            btn_frame, text="Thêm", command=add_selected, kind="accent", width=10
        ).pack(side="left")

        ModernButton(
            btn_frame, text="Hủy", command=dialog.destroy, kind="secondary", width=8
        ).pack(side="right")
    ...
    def _load_playlist(self):
        ...
        ModernButton(
            btn_frame, text="Mở", command=load_selected, kind="accent", width=8
        ).pack(side="left")
        ModernButton(
            btn_frame, text="Hủy", command=dialog.destroy, kind="secondary", width=8
        ).pack(side="right")
