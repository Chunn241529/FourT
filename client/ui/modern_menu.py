...
class ModernMenu(tk.Toplevel):
    ...
    def __init__(
        self, parent, x, y, items, on_close=None, animate=True, on_bug_report=None
    ):
        ...
        if animate:
            ...
            SlideEffect.slide_in(
                self, x, y, direction="left", distance=15, duration=120
            )
        else:
            self.geometry(f"+{x}+{y}")
    ...
    def _draw_background(self):
        ...
        for i in range(3, 0, -1):
            ...
            draw_rounded_rect(
                ...
                pad + i,
                pad + i,
                self.menu_width + pad + i,
                self.menu_height + pad + i,
        ...
        draw_rounded_rect(
            ...
            pad + 1,
            pad + 1,
            self.menu_width + pad - 1,
            self.menu_height + pad - 1,
    ...
    def _draw_header_icons(self):
        """Draw header icons row with logo left, language toggle and bug report right"""
        pad = 5
        ...
        # Language toggle icon - positioned left of bug icon
        lang_x = bug_x - 28
        lang_y = bug_y

        # Get current language and show appropriate flag
        current_lang = get_language()
        lang_icon = "🇻🇳" if current_lang == "vi" else "🇺🇸"
        ...
        self.canvas.create_text(
            ...
            font=("Segoe UI Emoji", 11),
            fill="#6c7086",  # Dim color by default
            tags=("lang_icon", "lang_clickable"),
        )

        # Bind hover and click events for bug icon
        ...
        # Bind hover and click events for language toggle
        self.canvas.tag_bind("lang_clickable", "<Enter>", self._on_lang_enter)
        self.canvas.tag_bind("lang_clickable", "<Leave>", self._on_lang_leave)
        self.canvas.tag_bind("lang_clickable", "<Button-1>", self._on_lang_click)

    def _on_lang_enter(self, event):
        """Language icon hover enter"""
        self.canvas.itemconfig("lang_icon", fill="#94e2d5")  # Teal color
        self.canvas.config(cursor="hand2")

    def _on_lang_leave(self, event):
        """Language icon hover leave"""
        self.canvas.itemconfig("lang_icon", fill="#6c7086")
        self.canvas.config(cursor="")

    def _on_lang_click(self, event):
        """Language icon click - toggle language without closing menu"""
        current = get_language()
        new_lang = "en" if current == "vi" else "vi"
        set_language(new_lang)

        # Update the icon to reflect new language
        new_icon = "🇻🇳" if new_lang == "vi" else "🇺🇸"
        self.canvas.itemconfig("lang_icon", text=new_icon)
    ...
    def _create_menu_item(self, idx: int, item: dict, pad: int, y: int):
        ...
        if (
            is_disabled
            and item.get("command") is None
            and "TRIAL" not in item.get("label", "")
        ):
        ...
        self.canvas.tag_bind(
            f"item_{idx}", "<Enter>", lambda e, i=idx: self._on_item_enter(i)
        )
        self.canvas.tag_bind(
            f"item_{idx}", "<Leave>", lambda e, i=idx: self._on_item_leave(i)
        )
        self.canvas.tag_bind(
            f"item_{idx}", "<Button-1>", lambda e, i=idx: self._on_item_click(i)
        )

        # Also bind to hover rect
        self.canvas.tag_bind(
            f"hover_{idx}", "<Enter>", lambda e, i=idx: self._on_item_enter(i)
        )
        self.canvas.tag_bind(
            f"hover_{idx}", "<Leave>", lambda e, i=idx: self._on_item_leave(i)
        )
        self.canvas.tag_bind(
            f"hover_{idx}", "<Button-1>", lambda e, i=idx: self._on_item_click(i)
        )
