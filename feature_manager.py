...
class FeatureManager:
    ...
    def _load_and_verify_license_cache(self) -> None:
        ...
        if self.skip_network:
            if self._is_within_grace_period(cache_data):
                ...
                print(f"[FeatureManager] Offline: using cached license ({self.current_package})")
            else:
    ...
    def _is_within_grace_period(self, cache_data: Dict) -> bool:
        ...
        try:
            ...
            if within_grace:
                ...
                print(f"[FeatureManager] Grace period: {days_remaining:.1f} days remaining")
            return within_grace
        except:
            return False
            ...
            if not is_server_offline():
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success"):
                        ...
                        self._clear_cache()
                        print(f"[FeatureManager] ✅ License activated (online): {self.current_package}")
                        return True
    ...
    def verify_license(self) -> bool:
        ...
        try:
            ...
            print(f"[Client] Sending POST request to: {get_license_endpoints()['verify']}")
            response = requests.post(
                ...
                if "expires_at" in self.license_data:
                    try:
                        ...
                        if datetime.now() < expires_at:
                            print(f"[FeatureManager] Using cached license (offline mode), expires: {expires_at}")
                            return True
                    except Exception:
                        pass
    ...
    def _check_trial_status(self) -> None:
        ...
        if self.trial_cache_timestamp and (time.time() - self.trial_cache_timestamp) < 300:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[FeatureManager] Trial: active={self.trial_active}, remaining={self.trial_remaining_seconds}s")
            else:
    ...
    def create_payment_order(self, package: str, amount: int = 0) -> Dict[str, Any]:
        ...
        if not is_server_offline():
            try:
                ...
                response = requests.post(
                    get_license_endpoints()["create_payment"],
                    json=payload,
                    timeout=5
                )
    ...
    def check_payment_status(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            import requests
            response = requests.get(f"{get_license_endpoints()['check_payment']}/{order_id}")
            if response.status_code == 200:
    ...
    def mock_payment_success(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            base_url = get_license_endpoints()["create_payment"].replace("/payment/create", "")
