...
from services.local_trial_manager import get_local_trial_manager
...
class FeatureManager:
    ...
    def __init__(self, skip_network: bool = False):
        ...
        # Local trial manager for offline trial support
        self.local_trial_manager = get_local_trial_manager()

        # Check trial (online first, fallback to offline)
    ...
    def _check_trial_status(self) -> None:
        """Check trial status from server, fallback to local if offline"""
        from services.connection_manager import is_server_offline
        ...
        # If offline or skip_network, use local trial
        if is_server_offline() or self.skip_network:
            self._check_local_trial()
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"[FeatureManager] Trial (server): active={self.trial_active}, remaining={self.trial_remaining_seconds}s"
                )
                return
            else:
        ...
        # Fallback to local trial on server error
        self._check_local_trial()

    def _check_local_trial(self) -> None:
        """Check and manage local offline trial"""
        result = self.local_trial_manager.check_trial()

        if result.get("active"):
            self.trial_active = True
            self.trial_remaining_seconds = result.get("remaining_seconds", 0)
            self.trial_cache_timestamp = time.time()
            print(
                f"[FeatureManager] Trial (local): active, remaining={self.trial_remaining_seconds}s"
            )
        elif result.get("can_start", False):
            # Start a new local trial
            if self.local_trial_manager.start_trial():
                self.trial_active = True
                self.trial_remaining_seconds = (
                    self.local_trial_manager.get_remaining_time()
                )
                self.trial_cache_timestamp = time.time()
                print(f"[FeatureManager] Started new local trial")
            else:
            ...
            self.trial_active = False
            self.trial_remaining_seconds = 0
            print(f"[FeatureManager] Local trial expired or invalid")
    ...
    def create_paypal_order(self, package: str) -> Dict[str, Any]:
        """
        Create PayPal order via Server API
        Returns: {order_id, approve_url, status}
        """
        from services.connection_manager import is_server_offline

        if is_server_offline():
            print("[FeatureManager] Cannot create PayPal order: Server offline")
            return {}

        try:
            import requests

            print(f"[Client] Creating PayPal order for {package}")
            response = requests.post(
                get_license_endpoints()["create_paypal"],
            ...
            if response.status_code == 200:
                ...
                print(f"[Payment] PayPal creation failed: {response.text}")

        except Exception as e:
            print(f"[Payment] PayPal creation error: {e}")
