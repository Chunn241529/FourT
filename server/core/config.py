...
WINDOW_WIDTH = 350
WINDOW_HEIGHT = 310
WINDOW_TITLE = "FourT - MIDI Auto-Player"
...
class Features:
    ...
    MACRO_UNLIMITED = "macro_unlimited" # Macro không giới hạn (infinite loop, save limit)
...
PACKAGE_FEATURES = {
    ...
    Packages.PLUS: [
        ...
        # Plus: Basic + Script Preview + Quest Video Helper
    ],
    Packages.PRO: [
        Features.MIDI_PLAYBACK,
        Features.LOOP_MODE,
        ...
        # Pro: Plus + Macro + MP3 Conversion + Ping Optimizer
    ],
    Packages.PREMIUM: [
        Features.MIDI_PLAYBACK,
        Features.LOOP_MODE,
        ...
        Features.MACRO,
        Features.MP3_CONVERSION,
        Features.MACRO_UNLIMITED,
        Features.WWM_COMBO,
        Features.QUEST_VIDEO_HELPER,
        Features.PING_OPTIMIZER,
        # Premium: Pro + Unlimited Macro + WWM Combo
    ],
}
...
SERVER_URL_CACHE_FILE = os.path.join(os.path.expandvars("%LOCALAPPDATA%"), "FourT", "server_url_cache.json")
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
                        ...
                        try:
                            with open(SERVER_URL_CACHE_FILE, 'w') as f:
                                json.dump({
                                    "server_url": url,
                                    "source": name,
                                    "all_urls": {k: data.get(k) for k, _ in url_keys if data.get(k)}
                                }, f)
                        except:
                            pass
                        return url
            ...
            for key, name in url_keys:
                ...
                if url:
                    ...
                    try:
                        with open(SERVER_URL_CACHE_FILE, 'w') as f:
                            json.dump({"server_url": url, "source": f"{name} (untested)"}, f)
                    except:
                        pass
                    return url
...
FEATURE_LIMITS = {
    ...
    Packages.PRO: {
        "midi_file_limit": 999,
        "macro_save_limit": 5,
        "macro_infinite_loop": False,
    },
    Packages.PREMIUM: {
        "midi_file_limit": 999,
        "macro_save_limit": 999,
        "macro_infinite_loop": True,
    },
}
